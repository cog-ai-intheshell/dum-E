...
const ROBOT_SHOULDER_Z = 0.76;
const ROBOT_UPPER_ARM_LENGTH = 1.55;
const ROBOT_FOREARM_LENGTH = 1.55;
const ROBOT_PALM_LENGTH = 0.16;
const ROBOT_GRIP_OFFSET = 0.22;
const ROBOT_REACH_LEAD_TIME = 0.7;
const REST_ROBOT_AXES = Object.freeze({
  axis_base: Math.PI,
  axis_elbow: 0.42,
  axis_wrist: -1.1,
});
...
// Keep the historical names for CSV/model compatibility while the playground concept evolves.
...
function readStoredWindVisualizationMode() {
  const storedMode = localStorage.getItem("dum-e-playground-wind-visualization") ||
    localStorage.getItem("basketball-simulator-wind-visualization");
...
function toggleWindVisualizationMode() {
  ...
  localStorage.setItem("dum-e-playground-wind-visualization", windVisualizationMode);
...
function updateWorldModelButton() {
  ...
  const catchPercent = hasMomentum ? Math.round(probability * 100) : null;
  const titles = {
    idle: "Show DUM-E World Model predictions",
    loading: "DUM-E World Model: prediction in progress",
    ready: hasMomentum
      ? `DUM-E World Model active: blue trajectory, ghost arms, catch ${catchPercent}%`
      : "DUM-E World Model active: predicted trajectory and ghost arms",
    offline: "World Model unavailable: run ./start_server.sh",
  };
  dom.worldModelToggle.textContent = worldModelEnabled && hasMomentum ? `C ${catchPercent}%` : "WM";
...
async function requestWorldModelPrediction(force = false) {
  ...
  if (isCapturedFrame(state.rows[state.frameIndex])) {
    ...
    state.worldModelCatchPlan = null;
    state.worldModelRobotMotion = [];
    state.worldModelControllerLoaded = false;
    state.worldModelMotionFrameIndex = null;
  ...
  try {
    ...
    state.worldModelCatchPlan = data.catch_plan || null;
    state.worldModelRobotMotion = Array.isArray(data.robot_motion) ? data.robot_motion : [];
    state.worldModelControllerLoaded = data.controller_loaded === true;
    state.worldModelMotionFrameIndex = state.frameIndex;
...
function closestSynchronizedPoints(a0, a1, b0, b1) {
  const aDelta = sub(a1, a0);
  const bDelta = sub(b1, b0);
  const relativeStart = sub(a0, b0);
  const relativeDelta = sub(aDelta, bDelta);
  ...
  const pointA = add(a0, scale(aDelta, alpha));
  const pointB = add(b0, scale(bDelta, alpha));
  return {
    alpha,
    pointA,
    pointB,
    distance: norm(sub(pointA, pointB)),
  };
}

function plannedRobotAxes(params, target, targetTime, time) {
  const startTime = Math.max(0, targetTime - ROBOT_REACH_LEAD_TIME);
  const progress = targetTime <= 1e-6 ? 1 : smoothstep(startTime, targetTime, time);
  return lerpRobotAxes(REST_ROBOT_AXES, robotAxesForTarget(params, target), progress);
}

function plannedGripPoint(params, target, targetTime, time) {
  return robotPoseFromAxes(params, plannedRobotAxes(params, target, targetTime, time)).gripPoint;
}

function applyCatcherMetrics(row, gripPoint, minDistance, captured = false) {
  const ball = [row.ball_x, row.ball_y, row.ball_z];
  const distance = norm(sub(ball, gripPoint));
  row.distance_ball_to_catcher = distance;
  row.horizontal_distance_to_catcher = Math.hypot(ball[0] - gripPoint[0], ball[1] - gripPoint[1]);
  row.min_catch_distance_so_far = minDistance;
  row.is_inside_hoop_radius = distance <= CATCH_RADIUS ? 1 : 0;
  row.is_captured = captured ? 1 : 0;
  row.held_by_dum_e = captured ? 1 : 0;
}

function isCapturedFrame(row) {
  return row && row.is_captured === 1;
}
...
function makeRow(params, step, t, position, velocity, acceleration, wind, minDistance, prevPosition) {
  ...
  const crossedPlane =
    prevPosition &&
    closestPointOnSegment(prevPosition, position, center).distance <= CATCH_RADIUS;
  ...
  return {
    ...
    distance_ball_to_catcher: distanceBallToHoop,
    horizontal_distance_to_hoop: horizontalDistanceToHoop,
    horizontal_distance_to_catcher: horizontalDistanceToHoop,
    speed_norm: speedNorm,
    is_crossing_hoop_plane: crossedPlane ? 1 : 0,
    is_inside_hoop_radius: distanceBallToHoop <= CATCH_RADIUS ? 1 : 0,
    is_descending: velocity[2] < 0 ? 1 : 0,
    min_hoop_distance_so_far: minDistance,
    min_catch_distance_so_far: minDistance,
    is_captured: 0,
    held_by_dum_e: 0,
    collision_time: null,
    capture_time: null,
    final_min_hoop_distance: null,
    final_min_catch_distance: null,
  };
}
...
function simulateShot(params) {
  ...
  // The UI label says "Initial force"; numerically it controls launch speed in m/s.
  ...
  const rawRows = [];
  let staticMinDistance = Infinity;
  let closestBallPosition = position.slice();
  let closestBallTime = 0;
  ...
  for (let step = 0; step < MAX_STEPS; step += 1) {
    ...
    const center = hoopCenter(params);
    const staticDistance = norm(sub(position, center));
    if (staticDistance < staticMinDistance) {
      staticMinDistance = staticDistance;
      closestBallPosition = position.slice();
      closestBallTime = time;
    }

    rawRows.push(makeRow(params, step, time, position, velocity, acceleration, wind, staticMinDistance, prevPosition));
  ...
  const robotTargetPosition = closestBallPosition.slice();
  const robotTargetTime = closestBallTime;
  const rows = [];
  let label = 0;
  ...
  for (let index = 0; index < rawRows.length; index += 1) {
    ...
    const row = { ...rawRow };
    const ball = [row.ball_x, row.ball_y, row.ball_z];
    const grip = plannedGripPoint(params, robotTargetPosition, robotTargetTime, row.time);
    const pointDistance = norm(sub(ball, grip));
    finalMinDistance = Math.min(finalMinDistance, pointDistance);

    if (index > 0) {
      const previousRow = rawRows[index - 1];
      const previousBall = [previousRow.ball_x, previousRow.ball_y, previousRow.ball_z];
      const previousGrip = plannedGripPoint(params, robotTargetPosition, robotTargetTime, previousRow.time);
      const movingClosest = closestSynchronizedPoints(previousBall, ball, previousGrip, grip);
      finalMinDistance = Math.min(finalMinDistance, movingClosest.distance);
      row.is_crossing_hoop_plane = movingClosest.distance <= CATCH_RADIUS ? 1 : 0;

      if (label === 0 && movingClosest.distance <= CATCH_RADIUS) {
        label = 1;
        collisionTime = lerp(previousRow.time, row.time, movingClosest.alpha);
        capturePosition = movingClosest.pointA;
        finalMinDistance = Math.min(finalMinDistance, movingClosest.distance);
        const captureGrip = movingClosest.pointB;
        const captureWind = windAt(
          capturePosition[0],
          capturePosition[1],
          capturePosition[2],
          collisionTime,
        ...
        applyCatcherMetrics(captureRow, captureGrip, finalMinDistance, true);
        rows.push(captureRow);
        break;
      }
    }

    applyCatcherMetrics(row, grip, finalMinDistance, false);
    rows.push(row);
  }
  ...
  for (const row of rows) {
    ...
    row.capture_time = collisionTime;
    row.final_min_hoop_distance = finalMinDistance;
    row.final_min_catch_distance = finalMinDistance;
    if (label === 1 && collisionTime != null && row.time >= collisionTime - 1e-9) {
      row.is_captured = 1;
      row.held_by_dum_e = 1;
      row.ball_vx = 0;
      row.ball_vy = 0;
...
function updateTitles() {
  ...
  const lastTimestep = state.rows[state.rows.length - 1]?.timestep ?? MAX_STEPS;
  dom.resultTitle.textContent = result;
  dom.resultTitle.className = result === RESULT_SUCCESS ? "result-made" : "result-fail";
  dom.timeTitle.innerHTML = `t = ${row.time.toFixed(2)} s &nbsp; (timestep ${row.timestep} / ${lastTimestep})`;
}
...
function renderResults() {
  ...
  const rows = [
    ...
    ["Distance to DUM-E", `${state.params.distanceToHoop.toFixed(1)} m`],
    ["Minimum distance to claw", `${state.metadata.minHoopDistance.toFixed(3)} m`],
    ["Catch time", collision],
    ...
    ["Horizontal orientation", `${state.params.windOrientation.toFixed(1)}°`],
    ["Vertical orientation", `${state.params.windVerticalOrientation.toFixed(1)}°`],
  ];
...
function makeShotCsvRow() {
  return {
    ...
    distance_to_dum_e: state.params.distanceToHoop,
    child_height: state.params.boyHeight,
    ball_mass: state.params.ballMass,
    catch_height: state.params.hoopHeight,
...
function makeTrajectoryCsvRow(row) {
  return {
    ...
    ball_ay: row.ball_ay,
    ball_az: row.ball_az,
    distance_ball_to_catcher: row.distance_ball_to_catcher,
    horizontal_distance_to_catcher: row.horizontal_distance_to_catcher,
...
function makeLabelCsvRow() {
  return {
    ...
    distance_to_dum_e: state.params.distanceToHoop,
    final_min_catch_distance: state.metadata.minHoopDistance,
    capture_time: state.metadata.collisionTime,
  };
}
...
function runSimulation(isInitial = false) {
  ...
  state = {
    ...
    worldModelCatchPlan: null,
    worldModelRobotMotion: [],
    worldModelControllerLoaded: false,
    worldModelMotionFrameIndex: null,
  };
...
function ringPoints(center, radius, plane = "xy", segments = 96) {
  const points = [];
...
function normalizeRobotAxes(axes = REST_ROBOT_AXES) {
  return {
    axis_base: Number.isFinite(axes?.axis_base) ? axes.axis_base : REST_ROBOT_AXES.axis_base,
    axis_elbow: Number.isFinite(axes?.axis_elbow) ? axes.axis_elbow : REST_ROBOT_AXES.axis_elbow,
    axis_wrist: Number.isFinite(axes?.axis_wrist) ? axes.axis_wrist : REST_ROBOT_AXES.axis_wrist,
  };
}

function robotAxesFromMotionStep(step) {
  if (!step) {
    return null;
  }
  return normalizeRobotAxes({
    axis_base: Number(step.axis_base),
    axis_elbow: Number(step.axis_elbow),
    axis_wrist: Number(step.axis_wrist),
  });
}

function currentRobotMotionCursor() {
  const motion = state?.worldModelRobotMotion;
  if (!worldModelEnabled || !Array.isArray(motion) || motion.length === 0) {
    return -1;
  }
  const startFrame = Number.isFinite(state.worldModelMotionFrameIndex)
    ? state.worldModelMotionFrameIndex
    : state.frameIndex;
  return clamp(state.frameIndex - startFrame + 1, 0, motion.length);
}

function currentRobotMotionIndex() {
  const motion = state?.worldModelRobotMotion;
  const cursor = currentRobotMotionCursor();
  if (cursor < 0 || !Array.isArray(motion) || motion.length === 0) {
    return -1;
  }
  return clamp(Math.max(0, Math.floor(cursor - 1)), 0, motion.length - 1);
}

function robotAxesForTarget(params, target) {
  const baseX = robotBaseX(params);
  const dx = target[0] - baseX;
  const dy = target[1] - TARGET_Y;
  const dz = target[2] - ROBOT_SHOULDER_Z;
  const horizontal = Math.hypot(dx, dy);
  const distance = Math.hypot(horizontal, dz);
  const effectiveForearmLength = ROBOT_FOREARM_LENGTH + ROBOT_PALM_LENGTH + ROBOT_GRIP_OFFSET;
  const maxReach = ROBOT_UPPER_ARM_LENGTH + effectiveForearmLength - 1e-6;
  const minReach = Math.abs(ROBOT_UPPER_ARM_LENGTH - effectiveForearmLength) + 1e-6;
  const clampedDistance = clamp(distance, minReach, maxReach);
  const baseYaw = Math.atan2(dy, dx);
  const targetPitch = Math.atan2(dz, Math.max(horizontal, 1e-6));
  const shoulderOffset = Math.acos(clamp(
    (ROBOT_UPPER_ARM_LENGTH ** 2 + clampedDistance ** 2 - effectiveForearmLength ** 2) /
      (2 * ROBOT_UPPER_ARM_LENGTH * clampedDistance),
    -1,
    1
  ));
  const forearmOffset = Math.acos(clamp(
    (effectiveForearmLength ** 2 + clampedDistance ** 2 - ROBOT_UPPER_ARM_LENGTH ** 2) /
      (2 * effectiveForearmLength * clampedDistance),
    -1,
    1
  ));
  const elbowPitch = targetPitch + shoulderOffset;
  const forearmPitch = targetPitch - forearmOffset;
  return {
    axis_base: baseYaw,
    axis_elbow: elbowPitch,
    axis_wrist: forearmPitch - elbowPitch,
  };
}

function defaultRobotAxes(params) {
  return robotAxesForTarget(params, catchCenter(params));
}
...
function lerpRobotAxes(from, to, t) {
  const progress = clamp(t, 0, 1);
  return {
    axis_base: from.axis_base + angleDelta(from.axis_base, to.axis_base) * progress,
    axis_elbow: lerp(from.axis_elbow, to.axis_elbow, progress),
    axis_wrist: lerp(from.axis_wrist, to.axis_wrist, progress),
  };
}

function scriptedRobotAxes(params) {
  if (!state?.rows?.length) {
    return REST_ROBOT_AXES;
  }
  const row = state.rows[state.frameIndex];
  const target = state.metadata.robotTargetPosition ||
    state.metadata.closestBallPosition ||
    state.metadata.capturePosition ||
    catchCenter(params);
  const targetTime = Number.isFinite(state.metadata.robotTargetTime)
    ? state.metadata.robotTargetTime
    : Number.isFinite(state.metadata.closestBallTime)
      ? state.metadata.closestBallTime
      : Number.isFinite(state.metadata.collisionTime)
        ? state.metadata.collisionTime
        : state.rows[state.rows.length - 1].time;
  return plannedRobotAxes(params, target, targetTime, row.time);
}

function currentRobotAxes(params) {
  // WM is an overlay: the real DUM-E arm keeps the same timeline as the normal simulation.
  return params ? scriptedRobotAxes(params) : REST_ROBOT_AXES;
}

function robotPoseFromAxes(params, axes = REST_ROBOT_AXES) {
  const normalizedAxes = normalizeRobotAxes(axes);
  const catchPoint = catchCenter(params);
  const baseX = robotBaseX(params);
  const shoulder = [baseX, TARGET_Y, ROBOT_SHOULDER_Z];
  const baseYaw = normalizedAxes.axis_base;
  const elbowPitch = normalizedAxes.axis_elbow;
  const forearmPitch = normalizedAxes.axis_elbow + normalizedAxes.axis_wrist;
  const shoulderDirection = [
    Math.cos(baseYaw) * Math.cos(elbowPitch),
    Math.sin(baseYaw) * Math.cos(elbowPitch),
    Math.sin(elbowPitch),
  ];
  const wristDirection = [
    Math.cos(baseYaw) * Math.cos(forearmPitch),
    Math.sin(baseYaw) * Math.cos(forearmPitch),
    Math.sin(forearmPitch),
  ];
  const sideDirection = normalize([-Math.sin(baseYaw), Math.cos(baseYaw), 0]);
  const upDirection = normalize(cross(wristDirection, sideDirection));
  const elbow = add(shoulder, scale(shoulderDirection, ROBOT_UPPER_ARM_LENGTH));
  const wrist = add(elbow, scale(wristDirection, ROBOT_FOREARM_LENGTH));
  const palm = add(wrist, scale(wristDirection, ROBOT_PALM_LENGTH));
  const clawCenter = add(palm, scale(wristDirection, ROBOT_GRIP_OFFSET));
  const claws = [
    add(clawCenter, add(scale(sideDirection, -0.18), scale(upDirection, 0.1))),
    add(clawCenter, add(scale(sideDirection, 0.18), scale(upDirection, 0.1))),
    add(clawCenter, add(scale(sideDirection, -0.16), scale(upDirection, -0.11))),
    add(clawCenter, add(scale(sideDirection, 0.16), scale(upDirection, -0.11))),
  ];
  return {
    baseX,
...
function drawRobotLink(ctx, projector, a, b, colors, width) {
  drawLine3D(ctx, projector, a, b, colors.robotArmShadow, width + 3.8, 0.96);
...
function drawRobotJoint(ctx, projector, point, radius, colors, label = "") {
  drawRing3D(ctx, projector, point, radius * 0.032, "xz", colors.robotJointStroke, 2.4, 0.92);
  drawCircleAt(ctx, projector, point, radius, colors.robotJoint, colors.robotJointStroke, 1.4);
  ...
  if (label) {
    ...
    drawText(ctx, label, projected.x + 24, projected.y - 18, {
      align: "left",
      color: colors.label,
...
function drawRobotGhostPose(ctx, projector, pose, colors, alpha, label) {
  drawLine3D(ctx, projector, pose.shoulder, pose.elbow, colors.worldPrediction, 4.2, alpha);
  drawLine3D(ctx, projector, pose.elbow, pose.wrist, colors.worldPrediction, 3.6, alpha);
  drawLine3D(ctx, projector, pose.wrist, pose.gripPoint, colors.catchZone, 2.8, alpha + 0.08);
  drawCircleAt(ctx, projector, pose.elbow, 3.4, colors.worldPredictionDot, colors.worldPredictionStroke, 0.8);
  drawCircleAt(ctx, projector, pose.wrist, 3.0, colors.worldPredictionDot, colors.worldPredictionStroke, 0.8);
  if (label) {
    const projected = projector.project(pose.gripPoint);
    drawText(ctx, label, projected.x + 10, projected.y + 10, {
      align: "left",
      color: colors.worldPrediction,
      font: "700 10px Inter, system-ui, sans-serif",
    });
  }
}

function drawRobotMotionGhosts(ctx, projector, params, colors) {
  const motion = state?.worldModelRobotMotion;
  if (!worldModelEnabled || !Array.isArray(motion) || motion.length === 0) {
    return;
  }
  const activeIndex = currentRobotMotionIndex();
  const baseIndex = Math.max(0, activeIndex);
  const checkpoints = [
    baseIndex + 4,
    baseIndex + 9,
    baseIndex + 14,
  ...
  for (const index of checkpoints) {
    const axes = robotAxesFromMotionStep(motion[index]);
    if (!axes) {
      continue;
    }
    const pose = robotPoseFromAxes(params, axes);
    const alpha = index === motion.length - 1 ? 0.42 : 0.24;
    drawRobotGhostPose(ctx, projector, pose, colors, alpha, `m+${index + 1}`);
  }
}

function drawDumERobot(ctx, projector, params) {
  const colors = canvasPalette();
  const pose = robotPoseFromAxes(params, currentRobotAxes(params));
  const { baseX, shoulder, elbow, wrist, palm, gripPoint } = pose;
  const platform = [
  ...
  const pedestalTop = [baseX, TARGET_Y, 0.56];

  drawPolyline3D(ctx, projector, platform, colors.robotBase, 7, 0.96);
  drawPolyline3D(ctx, projector, [
    [baseX - 0.3, TARGET_Y - 0.2, 0.09],
    [baseX + 0.3, TARGET_Y - 0.2, 0.09],
    [baseX + 0.34, TARGET_Y + 0.2, 0.09],
    [baseX - 0.34, TARGET_Y + 0.2, 0.09],
    [baseX - 0.3, TARGET_Y - 0.2, 0.09],
  ], colors.robotBaseEdge, 4, 0.9);
  drawLine3D(ctx, projector, [baseX, TARGET_Y - 0.2, 0.08], [baseX, TARGET_Y - 0.2, 0.56], colors.robotBase, 9, 0.96);
  drawLine3D(ctx, projector, [baseX, TARGET_Y + 0.2, 0.08], [baseX, TARGET_Y + 0.2, 0.56], colors.robotBase, 9, 0.96);
  drawRing3D(ctx, projector, [baseX, TARGET_Y, 0.16], 0.33, "xy", colors.robotBaseEdge, 3.2, 0.88);
  drawRing3D(ctx, projector, pedestalTop, 0.24, "xy", colors.robotJointAccent, 3, 0.96);

  drawDashedRing3D(ctx, projector, gripPoint, CATCH_RADIUS, "xy", colors.catchZone, 1.7, 0.74);
  drawDashedRing3D(ctx, projector, gripPoint, CATCH_RADIUS, "xz", colors.catchZoneSoft, 1.4, 0.56);
  drawRing3D(ctx, projector, gripPoint, CATCH_RADIUS * 0.58, "yz", colors.catchZoneSoft, 1.1, 0.34, colors.catchZoneGlow);
  drawRobotMotionGhosts(ctx, projector, params, colors);

  drawRobotLink(ctx, projector, shoulder, elbow, colors, 12);
  drawRobotLink(ctx, projector, elbow, wrist, colors, 10);
  drawLine3D(ctx, projector, [shoulder[0] + 0.05, shoulder[1] + 0.16, shoulder[2] + 0.08], [elbow[0] + 0.02, elbow[1] + 0.14, elbow[2] - 0.08], colors.robotCable, 1.6, 0.78);
  drawLine3D(ctx, projector, [elbow[0] - 0.04, elbow[1] + 0.14, elbow[2] + 0.06], [wrist[0] - 0.02, wrist[1] + 0.13, wrist[2] - 0.02], colors.robotCable, 1.5, 0.76);

  drawRobotJoint(ctx, projector, shoulder, 9.2, colors, "base");
  drawRobotJoint(ctx, projector, elbow, 8.6, colors, "elbow");
  drawRobotJoint(ctx, projector, wrist, 7.2, colors, "wrist");

  drawLine3D(ctx, projector, wrist, palm, colors.robotClaw, 5.4, 0.96);
  for (const claw of pose.claws) {
    drawLine3D(ctx, projector, palm, claw, colors.robotClaw, 3.4, 0.94);
  }
  drawCircleAt(ctx, projector, palm, 4.4, colors.robotJoint, colors.robotJointStroke, 1);

  const name = projector.project([baseX, TARGET_Y, 0.34]);
  drawText(ctx, "DUM-E", name.x, name.y - 24, {
    color: colors.robotJointAccent,
    font: "700 13px Inter, system-ui, sans-serif",
  });
  if (worldModelEnabled && state?.worldModelRobotMotion?.length) {
    const statusPoint = projector.project(palm);
    const source = state.worldModelControllerLoaded ? "controller" : "IK fallback";
    drawText(ctx, source, statusPoint.x + 16, statusPoint.y - 24, {
      align: "left",
      color: colors.catchZone,
...
function drawWorldModelPrediction(ctx, projector) {
  if (!worldModelEnabled || isCapturedFrame(state?.rows[state.frameIndex]) || !state?.worldModelPrediction?.length) {
    return;
  }
  const colors = canvasPalette();
  const row = state.rows[state.frameIndex];
  ...
  ctx.lineWidth = 1;
  for (let index = 1; index < points.length; index += 1) {
    const horizonIndex = index;
    const shouldLabel = horizonIndex <= 6 || horizonIndex % 5 === 0 || index === points.length - 1;
    const projected = projector.project(points[index]);
    ctx.beginPath();
    ctx.arc(projected.x, projected.y, shouldLabel ? 3.3 : 2.1, 0, TWO_PI);
    ctx.fill();
    ctx.stroke();
    if (shouldLabel) {
      drawText(ctx, `t+${horizonIndex}`, projected.x + 12, projected.y - 12, {
        align: "left",
        color: colors.worldPrediction,
        font: "700 11px Inter, system-ui, sans-serif",
      });
  ...
  const plan = state.worldModelCatchPlan;
  if (plan && Number.isFinite(plan.catch_x) && Number.isFinite(plan.catch_y) && Number.isFinite(plan.catch_z)) {
    const capture = projector.project([plan.catch_x, plan.catch_y, Math.max(plan.catch_z, 0)]);
    drawText(ctx, "WM catch", capture.x + 14, capture.y + 18, {
      align: "left",
      color: colors.worldPrediction,
    ...
    drawGlowingCircleAt(
      ctx,
      projector,
      [plan.catch_x, plan.catch_y, Math.max(plan.catch_z, 0)],
      5.8,
      colors.worldPredictionDot,
      colors.worldPredictionStroke,
      1.3,
      colors.worldPredictionGlow
    );
  }
}
...
function csvFilename(kind) {
  return `dum_e_throw_${state.params.shotId}_${kind}.csv`;
}
